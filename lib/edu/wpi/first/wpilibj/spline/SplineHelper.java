...
public final class SplineHelper {
    ...
    /**
     * Returns 2 cubic control vectors from a set of exterior waypoints and interior
     * translations.
     *
     ...
     * The user is free to set the direction of the start and end point. The
        ...
        if (waypoints.length > 1) {
            ...
            if (newWaypts.length > 4) {
                for (int i = 1; i <= newWaypts.length - 4; i++) {
                    ...
                    // of the second internal waypoint should involve the third and first internal
                    // waypoint,
                    // which have indices of 1 and 3 in the newWaypts list (which contains ALL
                    // waypoints).
     ...
     * vectors. The user is free to set the direction of all control vectors.
     * Continuous curvature is guaranteed throughout the path.
     *
    ...
    private static Spline.ControlVector getCubicControlVector(double scalar, Pose2d point) {
        return new Spline.ControlVector(new double[] { point.getX(), scalar * point.getRotation().getCos() },
    ...
    private static Spline.ControlVector getQuinticControlVector(double scalar, Pose2d point) {
        return new Spline.ControlVector(new double[] { point.getX(), scalar * point.getRotation().getCos(), 0.0 },
